...
print(np.median(SCARV_scores), np.median(CDTS_scores), np.median(Orion_scores))
...
scarv_assess.percScoreToCumulativePercCountPlot(SCARV_scores, axs[0])
scarv_assess.percScoreToCumulativePercCountPlot(CDTS_scores, axs[0])
scarv_assess.percScoreToCumulativePercCountPlot(Orion_scores, axs[0])

scarv_assess.percScoreToCumulativePercCountPlot(SCARV_scores, axs[1], 5)
scarv_assess.percScoreToCumulativePercCountPlot(CDTS_scores, axs[1], 5)
scarv_assess.percScoreToCumulativePercCountPlot(Orion_scores, axs[1], 5)

fig.tight_layout()
fig.subplots_adjust(bottom=0.28)
axs[0].legend(['SCARV', 'CDTS', 'Orion'], loc="lower center", bbox_to_anchor=(1.1, -0.45), ncol=5, prop={'size': 20}, frameon=False)
