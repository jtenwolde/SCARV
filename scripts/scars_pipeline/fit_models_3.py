...
sequence_snvs_and_dels = scars_queries.query_sequence(training_loci_subsample_snvs_and_dels, flank, genome, reference_fasta)
scars_queries.correct_refs(training_loci_subsample_snvs_and_dels, singleton_snvs, sequence_snvs_and_dels)
...
X_cal_snv_del, y_cal_snv_del, weight_cal_snv_del = scars_fit.prep_data(indices, output, calibration_multiplier, sequence_snvs_and_dels)
...
sequence_ins = scars_queries.query_sequence(training_loci_subsample_insertions, flank, genome, reference_fasta)
training_loci_subsample_insertions.End -= 1
...
X_cal_ins, y_cal_ins, weight_cal_ins = scars_fit.prep_data(indices_ins, output_ins, calibration_multiplier_ins, sequence_ins)
...
calibration_model_snvs_and_dels = scars_fit.fit_calibration(cnn, X_cal_snv_del, y_cal_snv_del, weight_cal_snv_del)
joblib.dump(calibration_model_snvs_and_dels, project_folder + "calibration/calibration_snv_del_" + str(2*flank+1) + ".h5")

calibration_model_ins = scars_fit.fit_calibration(cnn, X_cal_ins, y_cal_ins, weight_cal_ins)
